function run_afsacpso_research_cycle(numRuns, maxIterations)
%RUN_APEXPSO_RESEARCH_CYCLE Run the retained AFSACPSO ablation suite.
%
% This script benchmarks:
%   1) The retained full AFSACPSO stack.
...
% All variants are evaluated with paired seeds under a shared scenario set.
    ...
    ensureDir(outputDir);
    checkpointFile = fullfile(outputDir, 'afsacpso_checkpoint.csv');
    ...
    fprintf('==============================================\n');
    fprintf('  AFSACPSO Research Cycle Runner\n');
    fprintf('==============================================\n');
    ...
    writetable(allRuns, fullfile(outputDir, 'afsacpso_all_runs.csv'));
    writetable(summary, fullfile(outputDir, 'afsacpso_variant_summary.csv'));
    writeMarkdownSummary(summary, fullfile(outputDir, 'comparison.md'));

    fprintf('\nSaved files:\n');
    fprintf('  %s\n', fullfile(outputDir, 'afsacpso_all_runs.csv'));
    fprintf('  %s\n', fullfile(outputDir, 'afsacpso_variant_summary.csv'));
...
function variants = buildVariants(popSize, maxIterations)
    ...
    baseOverrides = applyParamModeOverrides(baseOverrides, 'attractor-field');
    ...
    variants(1) = makeVariant('APEX-Full', 'apex_full', 'core_loo', 'attractor-field', baseOverrides);
    variants(2) = makeVariant('LOO-NoAttractorFieldControl', 'loo_no_attractor_field_control', 'core_loo', 'global', ...
    ...
    variants(3) = makeVariant(sprintf('LOO-No%s', componentName), sprintf('loo_no_%s', componentId), ...
        'core_loo', 'attractor-field', mergeStruct(baseOverrides, componentOffOverrides));
end
...
function [fitnessValue, runtimeSeconds] = executeSingleRun(seed, maxIterations, variant, ...
    ...
    timerStart = tic;
    [~, stats] = callGlobalPlanningAlgorithm('AFSACPSO_Online', ...
...
function writeMarkdownSummary(summary, outputPath)
    ...
    fprintf(fid, '# AFSACPSO LOO Ablation Summary\n\n');
