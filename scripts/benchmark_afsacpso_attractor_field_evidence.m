function benchmark_afsacpso_attractor_field_evidence(numRuns, maxIterations, popSize, scenarios, numWorkers)
%BENCHMARK_AFSACPSO_ATTRACTOR_FIELD_EVIDENCE
% Run the missing attractor-field context and spread ablations and export
% table-ready rows for paper/main.tex.
    ...
    outputDir = fullfile(repoRoot, 'outputs', 'research', ...
        sprintf('afsacpso_attractor_field_evidence_%s', char(datetime('now', 'Format', 'yyyyMMdd_HHmmss'))));
    ...
    fprintf('============================================================\n');
    fprintf('AFSACPSO Attractor-Field Evidence Benchmark\n');
    fprintf('============================================================\n');
    fprintf('Runs: %d\n', numRuns);
...
function overrides = buildBaseOverrides(popSize)
    ...
    overrides.contextSpread = struct('w', 0.35, 'c1', 0.35, 'c2', 0.35);
end

function families = buildFamilies(baseOverrides)
    baseSpread = baseOverrides.contextSpread;

    contextVariants = {
        makeVariant('AFSACPSO', 'AFSACPSO', 'attractor-field', struct(), '')
        makeVariant('AFSACPSO (No Rank r)', 'AFSACPSO_NoRank', 'attractor-field', ...
            struct('rankResidualContextMask', struct('rank', false, 'velocity', true, 'stagnation', true)), ...
            'AFSACPSO (No Rank $r$)')
        makeVariant('AFSACPSO (No Velocity u)', 'AFSACPSO_NoVelocity', 'attractor-field', ...
            struct('rankResidualContextMask', struct('rank', true, 'velocity', false, 'stagnation', true)), ...
            'AFSACPSO (No Velocity $u$)')
        makeVariant('AFSACPSO (No Stagnation s)', 'AFSACPSO_NoStagnation', 'attractor-field', ...
            struct('rankResidualContextMask', struct('rank', true, 'velocity', true, 'stagnation', false)), ...
            'AFSACPSO (No Stagnation $s$)')
    };

    scaleVariants = {
        makeVariant('AFSACPSO', 'AFSACPSO', 'attractor-field', struct(), '')
        makeVariant('0.5x Context Spread', 'SpreadScale_0p5x', 'attractor-field', ...
            struct('contextSpread', scaleStruct(baseSpread, 0.5)), ...
            '$0.5\times$ Context Spread')
        makeVariant('1.5x Context Spread', 'SpreadScale_1p5x', 'attractor-field', ...
            struct('contextSpread', scaleStruct(baseSpread, 1.5)), ...
            '$1.5\times$ Context Spread')
    };

    families = [ ...
        struct('id', 'context_ablation', 'name', 'Context Ablation', 'variants', {contextVariants}); ...
        struct('id', 'context_spread_sensitivity', 'name', 'Context Spread Sensitivity', 'variants', {scaleVariants}) ...
    ];
end
...
function writeCombinedArtifacts(outputDir, combinedRows, numScenarios)
    csvPath = fullfile(outputDir, 'paper_attractor_field_missing_rows.csv');
    texPath = fullfile(outputDir, 'paper_attractor_field_missing_rows.tex');
