...
def main():
    ...
    if args.dir:
        ...
        # Default to the current project data directory based on main.tex usage
        script_dir = os.path.dirname(os.path.abspath(__file__))
        repo_root = os.path.dirname(script_dir)
        fig_dir = os.path.join(repo_root, "paper", "data", "rlbased")

    # Define standard targets and their default trims (L, B, R, T)
    trims = {
        "_TopView.pdf": {"left": 190, "bottom": 55, "right": 315, "top": 55},
    }
    ...
    # Filter pdf_files to ONLY include _TopView.pdf
    all_files = os.listdir(fig_dir)
    pdf_files = [f for f in all_files if f.lower().endswith("_topview.pdf")]
    ...
    for filename in pdf_files:
        ...
        if any_override:
            ...
            # This prevents compounding the base_trim in-place
    ...
    if processed_count == 0:
        print("No '_TopView.pdf' figures found to process.")
