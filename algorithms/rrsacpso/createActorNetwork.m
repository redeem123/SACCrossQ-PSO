    ...
    for i = 1:height(learnables)
        ...
        if strcmp(paramName, 'Weights')
            ...
            if endsWith(layerName, "gate_fc2")
                net.Learnables.Value{i} = dlarray(zeros(outputSize, inputSize, 'single'));
            else
                scale = sqrt(2.0 / inputSize);
                net.Learnables.Value{i} = dlarray(randn(outputSize, inputSize, 'single') * scale);
            end
        elseif strcmp(paramName, 'Bias')
            bias = learnables.Value{i};
            if endsWith(layerName, "gate_fc2")
                net.Learnables.Value{i} = dlarray(ones(size(bias), 'single') * 2.0);
            else
                net.Learnables.Value{i} = dlarray(zeros(size(bias), 'single'));
            end
        end
    end
end
