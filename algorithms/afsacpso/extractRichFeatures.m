function [basicFeatures, particles] = extractRichFeatures(particles, iter, maxIterations, lastImprovementIteration, globalBestFitness)
    ...
    % Cache expensive geometry for reuse by convertActionToPerParticleParams.
    % This avoids recomputing gbestPos, distances, and velocity magnitudes.
    if nargout >= 2 || true  % Always cache into particles struct
        particles.cachedGbestPos = globalBestPosition;
        particles.cachedDistToGbest = distancesToGbest;
        particles.cachedDirToGbest = directionToGbest;
        particles.cachedVelMag = velocityMagnitudes;
    end
end
