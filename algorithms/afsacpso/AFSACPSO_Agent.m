classdef AFSACPSO_Agent < handle
    % AFSACPSO SAC agent with retained two-component stack:
    ...
    methods
        function obj = AFSACPSO_Agent(config)
            % Constructor
            obj.config = config;
        ...
        function action = getAction(obj, state, training)
            ...
            if any(isnan(output(:))) || any(isinf(output(:)))
                warning('AFSACPSO:NaNDetected', 'NaN/Inf detected in actor network output. Resetting to zeros.');
            ...
            if any(isnan(action(:))) || any(isinf(action(:)))
                warning('AFSACPSO:NaNDetected', 'NaN/Inf detected in final action. Resetting to zeros.');
        ...
        function losses = train(obj)
            ...
            if obj.getConfigValue('useTQCCritic', false)
                ...
                critic1Grads = clipLearnablesGradients(critic1Grads, 1.0);
                ...
                critic2Grads = clipLearnablesGradients(critic2Grads, 1.0);
            ...
            if shouldUpdateActor
                ...
                actorGrads = clipLearnablesGradients(actorGrads, 1.0);
        ...
        function [actions, logProbs] = sampleAction(obj, states)
            ...
            % Check for NaN/Inf in network output
            if any(isnan(output(:))) || any(isinf(output(:)))
                warning('AFSACPSO:NaNDetected', 'NaN/Inf detected in sampleAction forward pass. Resetting to zeros.');
        ...
        function config = normalizeRuntimeConfig(~, config)
            % Research overrides are applied after AFSACPSO_Config runs, so
            ...
            if ~isfield(config, 'redqNumCritics')
                config.redqNumCritics = getFieldValue(config, 'numCritics', 5);
            end
            ...
            if ~isfield(config, 'redqPolicyUpdateDelay')
                config.redqPolicyUpdateDelay = getFieldValue(config, 'actorUpdateInterval', 5);
            end
            if ~isfield(config, 'redqWarmupSteps')
                config.redqWarmupSteps = getFieldValue(config, 'warmupPeriod', 128);
            end
            ...
            if config.useREDQCritic
                ...
                config.redqNumCritics = max(3, round(max( ...
                    getFieldValue(config, 'redqNumCritics', 5), ...
                    getFieldValue(config, 'numCritics', 5))));
                config.numCritics = config.redqNumCritics;
                config.redqTargetSubsetSize = max(2, min(config.numCritics, ...
                    round(getFieldValue(config, 'redqTargetSubsetSize', 2))));
                config.redqPolicyUpdateDelay = max(1, round(getFieldValue(config, ...
                    'redqPolicyUpdateDelay', getFieldValue(config, 'actorUpdateInterval', 5))));
                config.redqWarmupSteps = max(0, round(getFieldValue(config, ...
                    'redqWarmupSteps', getFieldValue(config, 'warmupPeriod', 128))));
                config.utdRatio = max(1, round(getFieldValue(config, 'utdRatio', 5)));
                config.gradientStepsPerTraining = max(1, round( ...
                    getFieldValue(config, 'gradientStepsPerTraining', config.utdRatio)));
                config.useDelayedPolicyUpdates = config.gradientStepsPerTraining > 1;
                if config.useDelayedPolicyUpdates
                    config.actorUpdateInterval = max(1, round(getFieldValue(config, ...
                        'actorUpdateInterval', config.gradientStepsPerTraining)));
                else
                    config.actorUpdateInterval = 1;
                end
                config.warmupPeriod = max(round(getFieldValue(config, 'warmupPeriod', 0)), ...
            ...
            elseif config.useCrossQCritic
                ...
                config.actorUpdateInterval = 1;
                config.numCritics = 2;
            elseif getFieldValue(config, 'useTQCCritic', false)
                config.useCrossQCritic = false;
                config.useREDQCritic = false;
                config.useAQECritic = false;
                config.useDroQCritic = false;
                ...
                config.utdRatio = 1;
                config.gradientStepsPerTraining = 1;
                config.tqcNumQuantiles = max(5, round(getFieldValue(config, 'tqcNumQuantiles', 25)));
                config.tqcDropQuantilesPerCritic = max(0, min(config.tqcNumQuantiles - 1, ...
                    round(getFieldValue(config, 'tqcDropQuantilesPerCritic', 2))));
                config.tqcHuberKappa = max(eps, getFieldValue(config, 'tqcHuberKappa', 1.0));
            elseif getFieldValue(config, 'useAQECritic', false)
                totalAqeHeads = max(1, round(getFieldValue(config, 'numCritics', 2))) * ...
                    max(1, round(getFieldValue(config, 'aqeHeadsPerCritic', 3)));
                config.aqeKeepHeads = max(1, min(totalAqeHeads, ...
                    round(getFieldValue(config, 'aqeKeepHeads', 2))));
            else
                config.numCritics = max(2, round(getFieldValue(config, 'numCritics', 2)));
                config.gradientStepsPerTraining = max(1, round( ...
                    getFieldValue(config, 'gradientStepsPerTraining', ...
                    getFieldValue(config, 'utdRatio', 1))));
                config.actorUpdateInterval = max(1, round(getFieldValue(config, 'actorUpdateInterval', 1)));
            end
        end
        ...
        function losses = trainREDQUpdate(obj, stateAction, statesDL, nextStatesDL, rewardsDL, donesDL, ...
            ...
            for criticIdx = 1:numCritics
                ...
                criticGrads = clipLearnablesGradients(criticGrads, 1.0);
        ...
        function saveWeights(obj, filePath)
            % Save actor weights for frozen inference deployment.
            % Only saves actor Learnables/State, logAlpha, obs norm stats,
            % and config metadata needed for architecture validation.
            obj.toCPU();

            % Extract actor Learnables (table → struct of plain arrays)
            learnables = obj.actor.Learnables;
            actorLearnables = struct();
            for i = 1:height(learnables)
                key = sprintf('%s__%s', learnables.Layer{i}, learnables.Parameter{i});
                key = strrep(key, '/', '_');
                val = learnables.Value{i};
                if isa(val, 'dlarray')
                    val = extractdata(val);
                end
                actorLearnables.(key) = gather(val);
            end

            % Extract actor State (table → struct of plain arrays)
            actorState = struct();
            if ~isempty(obj.actor.State) && height(obj.actor.State) > 0
                stateTable = obj.actor.State;
                for i = 1:height(stateTable)
                    key = sprintf('%s__%s', stateTable.Layer{i}, stateTable.Parameter{i});
                    key = strrep(key, '/', '_');
                    val = stateTable.Value{i};
                    if isa(val, 'dlarray')
                        val = extractdata(val);
                    end
                    actorState.(key) = gather(val);
                end
            end

            % Scalar parameters
            logAlphaVal = obj.logAlpha;
            if isa(logAlphaVal, 'dlarray')
                logAlphaVal = extractdata(logAlphaVal);
            end
            logAlphaVal = gather(logAlphaVal);

            % Observation normalization stats
            obsNorm = struct();
            obsNorm.count = obj.obsNormCount;
            obsNorm.mean = gather(obj.obsNormMean);
            obsNorm.M2 = gather(obj.obsNormM2);

            % Config metadata for architecture validation
            configMeta = struct();
            configMeta.actorHiddenLayers = obj.config.actorHiddenLayers;
            configMeta.actionSize = obj.config.actionSize;
            configMeta.stateSize = obj.config.stateSize;
            configMeta.paramMode = obj.config.paramMode;
            ...
            save(filePath, 'actorLearnables', 'actorState', 'logAlphaVal', ...
                'obsNorm', 'configMeta', '-v7.3');
            fprintf('  [saveWeights] Saved actor weights to %s\n', filePath);
        end

        function loadWeights(obj, filePath)
            % Load pre-trained actor weights for frozen inference.
            % Validates architecture compatibility before overwriting.
            if ~exist(filePath, 'file')
                error('AFSACPSO:WeightsNotFound', 'Weight file not found: %s', filePath);
            end

            data = load(filePath);

            % Validate architecture compatibility
            meta = data.configMeta;
            if ~isequal(meta.actorHiddenLayers, obj.config.actorHiddenLayers)
                error('AFSACPSO:ArchMismatch', ...
                    'actorHiddenLayers mismatch: file=[%s] vs config=[%s]', ...
                    num2str(meta.actorHiddenLayers), num2str(obj.config.actorHiddenLayers));
            end
            if meta.actionSize ~= obj.config.actionSize
                error('AFSACPSO:ArchMismatch', ...
                    'actionSize mismatch: file=%d vs config=%d', ...
                    meta.actionSize, obj.config.actionSize);
            end
            if meta.stateSize ~= obj.config.stateSize
                error('AFSACPSO:ArchMismatch', ...
                    'stateSize mismatch: file=%d vs config=%d', ...
                    meta.stateSize, obj.config.stateSize);
            end

            % Restore actor Learnables
            learnables = obj.actor.Learnables;
            fields = fieldnames(data.actorLearnables);
            for i = 1:height(learnables)
                key = sprintf('%s__%s', learnables.Layer{i}, learnables.Parameter{i});
                key = strrep(key, '/', '_');
                if ismember(key, fields)
                    obj.actor.Learnables.Value{i} = dlarray(data.actorLearnables.(key));
                else
                    warning('AFSACPSO:MissingWeight', 'Weight key %s not found in file', key);
                end
            end

            % Restore actor State
            if isfield(data, 'actorState') && ~isempty(fieldnames(data.actorState))
                stateTable = obj.actor.State;
                stateFields = fieldnames(data.actorState);
                for i = 1:height(stateTable)
                    key = sprintf('%s__%s', stateTable.Layer{i}, stateTable.Parameter{i});
                    key = strrep(key, '/', '_');
                    if ismember(key, stateFields)
                        obj.actor.State.Value{i} = dlarray(data.actorState.(key));
                    end
                end
            end

            % Restore logAlpha
            obj.logAlpha = data.logAlphaVal;

            % Restore observation normalization stats
            if isfield(data, 'obsNorm')
                obj.obsNormCount = data.obsNorm.count;
                obj.obsNormMean = data.obsNorm.mean;
                obj.obsNormM2 = data.obsNorm.M2;
            end

            fprintf('  [loadWeights] Loaded pre-trained actor from %s\n', filePath);
        end
