function particleParams = convertActionToPerParticleParams(action, config, particles, iter, maxIterations)
    ...
    %   config: AFSACPSO configuration struct
    %
    % Outputs:
    ...
    switch paramMode
        ...
        case 'rank-residual'
            % Direct SAC-to-parameter mapping with per-particle context.
            % No deterministic schedule — the SAC has full authority.
            %   a1,a4,a7 = population-level bias (one per param)
            %   a2,a3 / a5,a6 / a8,a9 = per-particle modulation via context
            % Cold start (action~0): w=0.5, c1=1.5, c2=1.5 (standard PSO).
            assert(length(action) == 9, 'Rank-residual mode expects 9D action');

            spread = struct('w', 0.35, 'c1', 0.35, 'c2', 0.35);
            if isfield(config, 'contextSpread')
                spread = config.contextSpread;
            end
            ...
            if ~isempty(particles)
                ...
                if isfield(particles, 'velocities') && ~isempty(particles.velocities)
                    if isfield(particles, 'cachedVelMag')
                        velMag = particles.cachedVelMag;
                    else
            ...
            % Inertia weight: a1=bias, a2=rank modulation, a3=velocity modulation
            wLevel = (action(1) + 1) / 2;
            wMod   = spread.w * (action(2) * rankCentered + action(3) * velCentered);
            particleParams(:, 1) = wMin + (wMax - wMin) * max(0, min(1, wLevel + wMod));

            % Cognitive coeff: a4=bias, a5=stagnation mod, a6=rank mod
            c1Level = (action(4) + 1) / 2;
            c1Mod   = spread.c1 * (action(5) * stagCentered + action(6) * rankCentered);
            particleParams(:, 2) = c1Min + (c1Max - c1Min) * max(0, min(1, c1Level + c1Mod));

            % Social coeff: a7=bias, a8=rank mod (negated), a9=stagnation mod (negated)
            c2Level = (action(7) + 1) / 2;
            c2Mod   = spread.c2 * (-action(8) * rankCentered - action(9) * stagCentered);
            particleParams(:, 3) = c2Min + (c2Max - c2Min) * max(0, min(1, c2Level + c2Mod));

        case 'attractor-field'
            % Attractor-Field Control Mode (v8.0).
            % Pure SAC authority with geometric per-particle context.
            % No deterministic schedule, no warmup.
            %
            % Three rank-normalized geometric signals describe each
            % particle's relationship to its PSO attractors (pbest, gbest).
            % Rank normalization prevents signal collapse near convergence.
            % Signal SHARING across parameters (attrTension in all three)
            % creates coherent per-particle profiles that are easier to learn.
            %
            % 9D action mapping:
            %   a1,a4,a7 = population-level bias (SAC controls overall level)
            %   a2,a3 / a5,a6 / a8,a9 = per-particle modulation via context
            %
            % Cold start (action~0): w≈0.5, c1=1.5, c2=1.5.
            assert(length(action) == 9, 'Attractor-field mode expects 9D action');

            spread = 0.40;
            if isfield(config, 'contextSpread') && isnumeric(config.contextSpread)
                spread = config.contextSpread;
            end

            attrTensionR  = zeros(numParticles, 1);
            velAlignR     = zeros(numParticles, 1);
            fitDriftR     = zeros(numParticles, 1);

            if ~isempty(particles) && numParticles > 1
                pos   = particles.cartesianPositions;
                vel   = particles.velocities;
                pbest = particles.bestPositions;
                pbestFit = particles.bestFitness(:);
                curFit   = particles.fitness(:);

                % Reuse cached geometry from extractRichFeatures if available
                if isfield(particles, 'cachedGbestPos')
                    gbestPos = particles.cachedGbestPos;
                    distToGbest = particles.cachedDistToGbest;
                else
                    validBestMask = isfinite(pbestFit);
                    if any(validBestMask)
                        validIdx = find(validBestMask);
                        [~, bestRelIdx] = min(pbestFit(validBestMask));
                        gbestIdx = validIdx(bestRelIdx);
                        gbestPos = pbest(gbestIdx, :);
                    else
                        gbestPos = mean(pos, 1);
                    end
                    distToGbest = sqrt(sum((pos - gbestPos).^2, 2));
                end

                % Geometric Signal 1: Attractor Tension (near gbest vs pbest)
                distToPbest = sqrt(sum((pos - pbest).^2, 2));
                rawTension = (distToGbest - distToPbest) ./ (distToGbest + distToPbest + 1e-8);

                % Geometric Signal 2: Velocity Alignment to Gbest
                dirToGbest = gbestPos - pos;
                velMag = sqrt(sum(vel.^2, 2));
                dirMag = sqrt(sum(dirToGbest.^2, 2));
                rawAlign = zeros(numParticles, 1);
                validMask = (velMag > 1e-8) & (dirMag > 1e-8);
                rawAlign(validMask) = sum(vel(validMask,:) .* dirToGbest(validMask,:), 2) ...
                                      ./ (velMag(validMask) .* dirMag(validMask));

                % Geometric Signal 3: Fitness Drift from Personal Best
                rawDrift = (curFit - pbestFit) ./ (abs(pbestFit) + 1);

                % Rank-normalize to [-0.5, +0.5]
                attrTensionR = rankNormalize(rawTension, numParticles);
                velAlignR    = rankNormalize(rawAlign, numParticles);
                fitDriftR    = rankNormalize(rawDrift, numParticles);
            end

            % Population-level bias from SAC.
            % Direct [-1,1] → [0,1] mapping (same as paper).
            wLevel  = (action(1) + 1) / 2;
            c1Level = (action(4) + 1) / 2;
            c2Level = (action(7) + 1) / 2;

            % Per-particle modulation via geometric signals.
            % Physics-informed defaults create role differentiation from
            % iteration 1 (cold start).  SAC learns residual corrections.
            if isfield(config, 'attractorFieldDefaults')
                d = config.attractorFieldDefaults;
            else
                d = struct('velAlignW',0, 'tensionW',0, 'driftC1',0, ...
                           'tensionC1',0, 'tensionC2',0, 'velAlignC2',0);
            end
            wMod  = spread * ((action(2)+d.velAlignW)*velAlignR  + (action(3)+d.tensionW)*attrTensionR);
            c1Mod = spread * ((action(5)+d.driftC1)*fitDriftR    + (action(6)+d.tensionC1)*attrTensionR);
            c2Mod = spread * (-(action(8)+d.tensionC2)*attrTensionR - (action(9)+d.velAlignC2)*velAlignR);
...
function centered = rankNormalize(values, N)
    % Map values to [-0.5, +0.5] via ranking.
    % Ties receive the average rank (fractional ranking).
    [~, order] = sort(values, 'ascend');
    ranks = zeros(N, 1);
    ranks(order) = (0:N-1)';
    centered = ranks / max(1, N - 1) - 0.5;
end
